...
import { Book, Clock, Library, Trophy, Flame, ChevronRight, Share2, AlertCircle, BookMarked, RefreshCw, Trash2, Headphones, Sparkles, BookOpen } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ 
    user, institute, level, onChangeCourse, language, onOpenVocabBook, onOpenMistakeBook, onClearMistakes, onStartModule 
}) => {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
          ...
          <div className="w-full md:w-auto bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex items-center justify-between gap-6 min-w-[300px]">
              <div className="flex items-center space-x-4">
                  ...
                  <div>
                      <h2 className="font-bold text-slate-800 text-sm uppercase tracking-wide">{institute ? institute.name : labels.noCourseSelected}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-red-200 transition-all text-left relative group overflow-hidden flex flex-col h-full">
                  ...
                  <div className="flex gap-2 relative z-10 mt-auto">
                      ...
                      {user.mistakes.length > 0 && (
                          <button 
                            onClick={(e) => { e.stopPropagation(); onClearMistakes(); }}
