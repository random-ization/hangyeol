...
const GrammarModule: React.FC<GrammarModuleProps> = ({ course, instituteName, language, levelContexts }) => {
  ...
  useEffect(() => {
    const fetchGrammar = async () => {
      ...
      const units = Object.keys(levelContexts).map(Number).sort((a,b) => a-b);
      ...
      for (const unit of units) {
          try {
              ...
              const data = await generateGrammarLesson(instituteName, course.level, unit, language, content);
              if (data.length > 0) {
                  newGroups[unit] = data;
              }
          } catch(e) {
  ...
  const filteredData = useMemo(() => {
      ...
      Object.keys(groupedPoints).forEach(unitKey => {
          ...
          const matches = points.filter(p => 
              ...
              p.usages.some(u => u.example.toLowerCase().includes(query) || u.translation.toLowerCase().includes(query))
          );
  ...
  const units = Object.keys(filteredData).map(Number).sort((a,b) => a-b);
  const totalPoints = Object.values(filteredData).reduce((acc: number, curr: GrammarPoint[]) => acc + curr.length, 0);
    ...
    <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
             <div>
                ...
                <p className="text-slate-500 text-sm mt-1">{instituteName} Level {course.level} • {totalPoints} Points</p>
             </div>
            ...
            {units.map(unit => (
                <div key={unit}>
                    <div className="flex items-center mb-6">
                        <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-sm font-bold mr-4">{labels.unit} {unit}</span>
                        <div className="h-px bg-slate-200 flex-1"></div>
                    </div>
                    
                    <div className="grid gap-6">
                        {filteredData[unit].map((point, idx) => (
                            <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:border-indigo-300 transition-colors">
                            ...
                            <div className="p-6">
                                ...
                                <div className="space-y-4">
                                    {point.usages.map((usage, uIdx) => (
                                        <div key={uIdx} className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                                            <div className="flex items-center mb-2">
                                                <span className="text-xs font-bold text-white bg-slate-400 px-2 py-0.5 rounded-full mr-2">{usage.situation}</span>
                                            </div>
                                            <p className="font-medium text-slate-800 text-lg mb-1">{usage.example}</p>
