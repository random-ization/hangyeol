...
const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, currentPage, onNavigate, language, onLanguageChange }) => {
        ...
        <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            >
              ...
              <span className="text-xl font-bold text-slate-800 tracking-tight hidden sm:block">HanGyeol</span>
            </div>
            ...
            <nav className="hidden md:flex space-x-1">
              {navItems.map(item => (
                ...
                >
                  <item.icon className={`w-4 h-4 mr-2 ${currentPage === item.id ? 'text-indigo-600' : 'text-slate-400'}`} />
                  {item.label}
                </button>
              ))}
            </nav>
