...
const ListeningModule: React.FC<ListeningModuleProps> = ({ course, instituteName, onSaveAnnotation, annotations, language, levelContexts }) => {
  ...
  const contextKey = activeUnit ? `${course.instituteId}-${course.level}-${activeUnit}-LISTENING` : '';
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        ...
        const data = await generateReadingPassage(instituteName, course.level, activeUnit, language, effectiveContent);
        setPassage(data);
      } catch (error) {
  ...
  if (!activeUnit) {
      ...
      return (
          <div className="max-w-4xl mx-auto">
              <h2 className="text-2xl font-bold text-slate-800 mb-6">{labels.toc} - {labels.listening}</h2>
              {unitsWithAudio.length === 0 ? (
                  ...
                  <div className="space-y-4">
                      {unitsWithAudio.map(u => {
                          const c = levelContexts[u];
                          const title = c.listeningScript ? c.listeningScript.substring(0, 50) + '...' : `Track ${u}`;
                          return (
                              <button 
                                key={u}
    ...
    <div className="max-w-2xl mx-auto">
      {/* Header */}
       <div className="mb-6 flex items-center">
            ...
            <span className="text-sm font-bold text-slate-700">{labels.unit} {activeUnit}</span>
       </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-100 relative">
        <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-slate-800 mb-2">{passage?.title || labels.listeningExercise}</h2>
                    ...
                    <div className="text-lg leading-relaxed text-slate-800 mb-4 whitespace-pre-line">
                        {sentences.map((sentence, idx) => {
                            ...
                            return (
                                ...
                                >
                                    {sentence}{' '}
                                    {hasNote && <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-400 rounded-full"></span>}
                                </span>
                            );
                        })}
                    </div>
