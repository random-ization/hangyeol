...
const ReadingModule: React.FC<ReadingModuleProps> = ({ 
    course, instituteName, onSaveWord, onSaveAnnotation, savedWordKeys, annotations, language, levelContexts 
}) => {
  ...
  const [currentSelectionRange, setCurrentSelectionRange] = useState<{start: number, end: number, text: string} | null>(null);
  ...
  const contextKey = activeUnit ? `${course.instituteId}-${course.level}-${activeUnit}-READING` : '';
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        const data = await generateReadingPassage(instituteName, course.level, activeUnit, language, content);
        setPassage(data);
      } catch (error) {
        console.error(error);
      } finally {
  ...
  useEffect(() => {
      const handleClickOutside = (event: MouseEvent) => {
          if (showAnnotationMenu && contentRef.current && !contentRef.current.contains(event.target as Node)) {
  ...
  const handleTextSelection = () => {
      ...
      const exactMatch = currentAnnotations.find(a => 
          Math.abs((a.startOffset || 0) - start) < 2 && Math.abs((a.endOffset || 0) - end) < 2
      );
  ...
  const saveHighlight = (color: Annotation['color']) => {
      ...
      const exactMatch = currentAnnotations.find(a => 
         Math.abs((a.startOffset || 0) - currentSelectionRange.start) < 2 && Math.abs((a.endOffset || 0) - currentSelectionRange.end) < 2
      );
  ...
  if (!activeUnit) {
      const availableUnits = Object.keys(levelContexts).map(Number).sort((a,b) => a-b);
      const unitsWithReading = availableUnits.filter(u => !!levelContexts[u]?.readingText);

      return (
          <div className="max-w-4xl mx-auto">
              <h2 className="text-2xl font-bold text-slate-800 mb-6">{labels.toc} - {labels.reading}</h2>
                          ...
                          return (
                              ...
                              >
                                  <div>
                                      ...
                                      <p className="text-sm text-slate-500 mt-2 line-clamp-2">{content.readingText.substring(0, 100)}...</p>
                                  </div>
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col max-w-7xl mx-auto w-full">
        ...
        <div className="mb-4 flex items-center">
            ...
            <span className="text-sm font-bold text-slate-700">{labels.unit} {activeUnit}: {passage.title}</span>
        </div>
                    ...
                    {showAnnotationMenu && menuPosition && (
                        ...
                        >
                            ...
                            <div className="flex space-x-2 mb-4">
                                <button onClick={() => saveHighlight('yellow')} className="w-6 h-6 rounded-full bg-[#fef08a] border border-slate-200 hover:scale-110 transition-transform"></button>
                                <button onClick={() => saveHighlight('green')} className="w-6 h-6 rounded-full bg-[#bbf7d0] border border-slate-200 hover:scale-110 transition-transform"></button>
                                <button onClick={() => saveHighlight('blue')} className="w-6 h-6 rounded-full bg-[#bfdbfe] border border-slate-200 hover:scale-110 transition-transform"></button>
                                <button onClick={() => saveHighlight('pink')} className="w-6 h-6 rounded-full bg-[#fbcfe8] border border-slate-200 hover:scale-110 transition-transform"></button>
                                <button onClick={() => saveHighlight(null)} className="w-6 h-6 rounded-full bg-white border border-slate-200 flex items-center justify-center text-xs text-slate-400 hover:text-red-500">✕</button>
                            </div>
                        ...
                        {sidebarAnnotations.map((ann) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-2">
                                    {/* Color Indicator */}
                                    <div className={`w-3 h-3 rounded-full mt-1 bg-${ann.color || 'slate'}-300 border border-slate-100`}></div>
                                    <button 
