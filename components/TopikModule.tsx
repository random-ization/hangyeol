...
const TopikModule: React.FC<TopikModuleProps> = ({
  ...
  const sidebarAnnotations = annotations.filter(
  ...
  const renderAnnotatedText = (text: string | undefined, subContextKey: string) => {
    ...
    const keyAnns = annotations
      .filter(
        a =>
          a.contextKey === fullContextKey &&
    ...
    while (i < text.length) {
      ...
      const char = text[i];
      const ann = keyAnns.find(
        a => cleanIndex >= (a.startOffset || 0) && cleanIndex < (a.endOffset || 0)
      );
    ...
    while (currentSegIndex < cleanSegments.length) {
      ...
      const segmentText = cleanSegments
        .slice(currentSegIndex, endNodeIndex)
        .map(s => s.char)
        .join('');
      const ann = startNode.annotation;
      let className = '';
      if (startNode.bold) className += 'font-bold ';
      if (startNode.underline)
        className += 'border-b-2 border-slate-900 pb-0.5 inline-block leading-normal ';
      if (ann) {
    ...
    return (
      <div
        data-context-key={fullContextKey}
        onMouseUp={() => handleTextSelection(fullContextKey)}
  ...
  const renderQuestionBlock = (
    q: TopikQuestion,
    showPassage: boolean,
    isGroupedChild: boolean = false
  ) => {
    const isReview = view === 'REVIEW';
    const activeAnswers =
      isReview && currentReviewAttempt ? currentReviewAttempt.userAnswers : userAnswers;
    ...
    return (
      <div
        ...
        className={`flex gap-4 ${isGroupedChild ? 'mb-8' : 'mb-12'}`}
      >
        <div className="text-lg font-sans text-slate-900 min-w-[28px] pt-0.5">{q.id}.</div>
        <div className="flex-1 w-full min-w-0">
          ...
          {hasImageOptions ? (
              ...
              {q.optionImages!.map((img, optIdx) => {
                ...
                if (isReview) {
                  ...
                  if (userAnswer === optIdx)
                    borderClass = 'border-indigo-600 ring-2 ring-indigo-100';
                }
                ...
                return (
                  ...
                  >
                    <div
                      className={`absolute top-2 left-2 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold z-10 ${userAnswer === optIdx || (isReview && optIdx === q.correctAnswer) ? 'bg-indigo-600 text-white' : 'bg-slate-100/90 text-slate-600 shadow-sm'}`}
                    >
            ...
            <div
              className={`grid ${q.options.some(o => o.length > 25) ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'} gap-x-8 gap-y-2`}
            >
              {q.options.map((opt, optIdx) => {
                ...
                if (isReview) {
                  if (optIdx === q.correctAnswer)
                    ...
                    optionClass = 'bg-red-100 border-red-500 text-red-900 font-bold';
                ...
                return (
                  <label
                    key={optIdx}
                    className={`flex items-start cursor-pointer p-2 rounded -ml-2 transition-colors border ${optionClass}`}
                  >
                    <input
                      type="radio"
                      name={`q-${q.id}`}
                    ...
                    />
                    <span
                      className={`text-lg mr-2 font-sans ${userAnswer === optIdx ? 'text-black font-bold' : 'text-slate-500'}`}
                    >
                      {getCircleNumber(optIdx)}
                    </span>
                    <span
                      className={`text-[17px] leading-relaxed mt-0.5 ${userAnswer === optIdx && !isReview ? 'text-indigo-900 font-bold underline decoration-indigo-300 underline-offset-4' : 'text-slate-800'}`}
                    >
                      {opt}
                    </span>
                    {isReview && optIdx === q.correctAnswer && (
    ...
    while (i < currentExam.questions.length) {
      ...
      if (q.instruction) {
        nodes.push(
          <div
            key={`inst-${q.id}`}
            className="mb-6 font-bold text-slate-800 text-[17px] bg-slate-100/50 p-2 rounded border-l-4 border-slate-800"
          >
            {q.instruction}
          </div>
        );
        ...
        <div className="max-w-[850px] mx-auto flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center text-sm font-bold text-slate-500 hover:text-indigo-600"
            >
            ...
            <div className="text-sm font-medium bg-slate-100 px-3 py-1 rounded-full">
              {labels.score}:{' '}
              <span className="text-indigo-600 font-bold">{currentReviewAttempt.score}</span> /{' '}
              {currentReviewAttempt.maxScore}
            </div>
          </div>
              ...
              <div className="flex items-center gap-2 bg-red-50 px-2 py-1 rounded border border-red-100">
                ...
                {wrongQs.map(id => (
                  <button
                    ...
                    className="w-6 h-6 flex items-center justify-center rounded-full bg-white border border-red-200 text-red-600 hover:bg-red-600 hover:text-white transition-colors font-medium"
                  >
                    {id}
                  </button>
                ))}
              ...
              <div className="flex items-center gap-2 bg-green-50 px-2 py-1 rounded border border-green-100">
                ...
                {correctQs.map(id => (
                  <button
                    key={id}
                    onClick={() => scrollToQuestion(id)}
                    className="w-6 h-6 flex items-center justify-center rounded-full bg-white border border-green-200 text-green-600 hover:bg-green-600 hover:text-white transition-colors font-medium"
                  >
                    {id}
                  </button>
                ))}
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-sm text-slate-500">
                          {exam.questions.length} {labels.questions} • {exam.timeLimit} min
                        </p>
                      </div>
                      ...
                      {hasAttempts ? (
                        <>
                          <button
                            ...
                            className="flex-1 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold rounded-lg transition-colors flex items-center justify-center"
                          >
                            <RotateCcw className="w-4 h-4 mr-2" />
                            {labels.retake}
                          </button>
                          <button
                            onClick={() => viewHistoryList(exam.id)}
                            className="flex-1 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-lg transition-colors flex items-center justify-center"
                          >
                        ...
                        <button
                          onClick={() => selectExam(exam)}
                          className="w-full py-2 bg-slate-50 hover:bg-blue-600 hover:text-white text-slate-700 font-medium rounded-lg transition-colors"
                        >
                          {labels.startExam}
                        </button>
                      )}
            ...
            <div className="space-y-4">
              {listeningExams.map(exam => {
                ...
                return (
                  <div
                    key={exam.id}
                    className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:border-indigo-300 transition-all group"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                    ...
                    <div className="flex gap-3 mt-4">
                      {hasAttempts ? (
                        ...
                        <button
                          onClick={() => selectExam(exam)}
                          className="w-full py-2 bg-slate-50 hover:bg-indigo-600 hover:text-white text-slate-700 font-medium rounded-lg transition-colors"
                        >
                          {labels.startExam}
                        </button>
                      )}
  ...
  if (view === 'HISTORY_LIST' && currentExam) {
    ...
    const attempts = history
      .filter(h => h.examId === currentExam.id)
      .sort((a, b) => b.timestamp - a.timestamp);
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <button
          onClick={() => setView('LIST')}
          className="flex items-center text-slate-500 hover:text-indigo-600 mb-4 font-bold"
        >
        ...
        <div className="bg-white rounded-xl p-8 border border-slate-200 shadow-sm">
          ...
          <div className="space-y-3">
            {attempts.map((attempt, idx) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="bg-white p-2 rounded-full border border-slate-200 text-slate-400 font-mono text-sm font-bold">
                    #{attempts.length - idx}
                  </div>
        ...
        <div className="bg-white w-full max-w-[800px] shadow-2xl min-h-[1000px] p-12 border border-slate-300 relative">
          <div className="border-2 border-black p-8 h-full flex flex-col justify-between">
            ...
            <div className="space-y-8 font-serif leading-relaxed text-lg text-slate-900">
              <div>
                <p className="mb-1">1. 시험 시작 지시가 있을 때까지 문제를 풀지 마십시오.</p>
                <p className="text-sm text-slate-600 font-sans">{labels.topikInstructions[1]}</p>
              </div>
              <div>
                <p className="mb-1">2. 수험번호와 이름을 정확하게 적어 주십시오.</p>
                <p className="text-sm text-slate-600 font-sans">{labels.topikInstructions[2]}</p>
              </div>
              <div>
                <p className="mb-1">3. 답안지를 구기거나 훼손하지 마십시오.</p>
                <p className="text-sm text-slate-600 font-sans">{labels.topikInstructions[3]}</p>
              </div>
              <div>
                <p className="mb-1">
                  4. 답안지의 이름, 수험번호 및 정답의 기입은 배부된 펜을 사용하여 주십시오.
                </p>
                <p className="text-sm text-slate-600 font-sans">{labels.topikInstructions[4]}</p>
              </div>
            </div>
            <div className="mt-20 flex justify-center">
              <button
                onClick={startExam}
                className="px-12 py-4 bg-black text-white text-xl font-bold hover:bg-slate-800 transition-colors shadow-lg"
              >
                {labels.startExam}
              </button>
            </div>
    ...
    return (
      <div className="flex flex-col h-screen bg-slate-200 overflow-hidden relative">
        ...
        {view === 'EXAM' && (
          <div className="bg-slate-900 text-white h-14 flex justify-between items-center px-6 shadow-md z-20 shrink-0">
            <div className="flex items-center space-x-4">
              <span className="font-bold text-lg">{currentExam.title}</span>
            </div>
            <div
              className={`flex items-center text-xl font-mono font-bold ${timeLeft < 300 ? 'text-red-400 animate-pulse' : 'text-emerald-400'}`}
            >
            ...
            <div className="flex items-center space-x-3">
              <button
                onClick={submitExam}
                className="px-4 py-1.5 bg-white text-slate-900 rounded font-bold hover:bg-slate-100 text-sm"
              >
                {labels.submitExam}
              </button>
            </div>
        ...
        <div className="flex flex-1 overflow-hidden relative flex-row">
          {/* PAPER CONTAINER */}
          <div
            className={`flex-1 overflow-y-auto p-4 lg:p-8 flex justify-center bg-slate-200 ${view === 'EXAM' ? 'select-none' : 'select-text'}`}
          >
            {/* THE PAPER */}
            <div className="bg-white w-full max-w-[850px] shadow-xl h-max min-h-[1200px] pb-20 border border-slate-300 relative text-black">
              ...
              <div className="pt-12 px-12 pb-6 text-center border-b-2 border-black mb-8">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-sm text-slate-500">
                    제{currentExam.round}회 한국어능력시험
                  </span>
                </div>
                <div className="border-2 border-black rounded-full inline-block px-12 py-2 mb-6">
                  <h1 className="text-3xl font-extrabold tracking-widest font-sans">
                    TOPIK Ⅱ{' '}
                    <span className="bg-black text-white rounded-full px-2 ml-2 text-2xl">B</span>
                  </h1>
                </div>
                <div className="flex justify-center">
                  <div className="flex border border-slate-400 bg-slate-100">
                    ...
                    <div className="px-12 py-2 font-bold text-lg bg-slate-100">
                      {currentExam.type === 'READING' ? '읽 기' : '듣 기'}
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* QUESTIONS AREA */}
              <div className="px-8 lg:px-16 font-serif">{renderExamContent()}</div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {sidebarAnnotations.map(ann => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div
                        className="w-3 h-3 rounded-full mt-1 border border-slate-100"
                        style={{
                          backgroundColor:
                            ann.color === 'yellow'
                              ? '#fef08a'
        ...
        <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-8 text-center animate-in zoom-in-95 duration-300">
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition-colors shadow-lg shadow-indigo-200 flex items-center justify-center"
            >
              <Eye className="w-5 h-5 mr-2" /> {labels.inspectExam}
            </button>
            <button
              onClick={() => setView('LIST')}
