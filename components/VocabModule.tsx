...
const VocabModule: React.FC<VocabModuleProps> = ({ 
  ...
  const getPosStyle = (pos: string | undefined) => {
      ...
      // Nouns/Pronouns (Blue)
      if (lower.includes('名') || lower.includes('noun') || lower.includes('代') || lower.includes('pronoun')) return 'bg-blue-100 text-blue-700';
  ...
  const handleWritingSubmit = (e?: React.FormEvent) => {
      ...
      let correct = '';
      if (currentQuestionType === 'WRITING_N_TO_K') correct = currentLearnItem.korean.replace(/\s+/g, '');
      else correct = currentLearnItem.english.replace(/\s+/g, '').toLowerCase(); 
      
      if (settings.flashcard.autoTTS) speak(currentLearnItem.korean);

      const isCorrect = input === correct || (currentQuestionType === 'WRITING_K_TO_N' && currentLearnItem.english.toLowerCase().includes(input));
  ...
  const handleCardRate = useCallback((known: boolean) => {
  ...
  useEffect(() => {
    if (viewMode === 'CARDS' && !isSessionComplete) {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === ' ' || e.key === 'ArrowUp' || e.key === 'ArrowDown') setIsFlipped(prev => !prev);
        };
  ...
  const Switch = ({ checked, onChange, label }: { checked: boolean, onChange: () => void, label?: string }) => (
    <div className="flex items-center justify-between py-3">
        ...
        >
            <div className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform duration-200 ${checked ? 'translate-x-6' : 'translate-x-1'}`} />
        </button>
    </div>
  );
              ...
              <div className="p-6 overflow-y-auto">
                  {settingsTab === 'FLASHCARD' && (
                      <div className="space-y-6">
                          {/* Flashcard Specific */}
                          <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{labels.flashcards}</label>
                                
                                <Switch 
                                    label={labels.shuffle} 
                                    checked={settings.flashcard.random} 
                                    onChange={() => setSettings(s => ({...s, flashcard: {...s.flashcard, random: !s.flashcard.random}}))}
                                />
                                <Switch 
                                    label={labels.autoTTS} 
                                    checked={settings.flashcard.autoTTS} 
                                    onChange={() => setSettings(s => ({...s, flashcard: {...s.flashcard, autoTTS: !s.flashcard.autoTTS}}))}
                                />
                                ...
                                <div className="mt-4">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <div className="flex bg-slate-100 p-1 rounded-lg">
                                            <button 
                                                onClick={() => setSettings(s => ({...s, flashcard: {...s.flashcard, cardFront: 'KOREAN'}}))}
                                            ...
                                            <button 
                                                onClick={() => setSettings(s => ({...s, flashcard: {...s.flashcard, cardFront: 'NATIVE'}}))}
                                ...
                                <div className="mt-4">
                                    ...
                                    <input 
                                        ...
                                        onChange={(e) => setSettings(s => ({...s, flashcard: {...s.flashcard, batchSize: parseInt(e.target.value)}}))}
                                        className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                                    />
                                    <div className="flex justify-between text-xs text-slate-400 mt-1">
                                        <span>5</span>
                                        <span className="font-bold text-indigo-600">{settings.flashcard.batchSize}</span>
                                        <span>50</span>
                                    </div>
                                </div>
                          </div>
                  ...
                  {settingsTab === 'LEARN' && (
                      <div className="space-y-6">
                           <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{labels.learn}</label>
                                
                                <Switch 
                                    label={labels.shuffle} 
                                    checked={settings.learn.random} 
                                    onChange={() => setSettings(s => ({...s, learn: {...s.learn, random: !s.learn.random}}))}
                                />
                                ...
                                <div className="mt-4">
                                    ...
                                    <input 
                                        ...
                                        onChange={(e) => setSettings(s => ({...s, learn: {...s.learn, batchSize: parseInt(e.target.value)}}))}
                                        className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                                    />
                                    <div className="flex justify-between text-xs text-slate-400 mt-1">
                                        <span>5</span>
                           ...
                           <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{labels.questionTypes}</label>
                                <Switch 
                           ...
                           <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{labels.answers}</label>
                                <Switch 
                                    label={labels.korean} 
    ...
    <div className="w-full max-w-[1920px] mx-auto">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
             <div>
                <h2 className="text-2xl font-bold text-slate-800">{customWordList ? (customListType === 'SAVED' ? labels.vocabBook : labels.mistakeBook) : labels.vocab}</h2>
                <p className="text-sm text-slate-500">{filteredWords.length} {labels.term}</p>
             </div>
             <button 
                onClick={() => {
        ...
        <div className="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                ...
                <div className="relative flex-1 lg:flex-none min-w-[200px]">
                    ...
                    >
                        ...
                        {availableUnits.map(u => (
                            <option key={u} value={u}>{labels.unit} {u}</option>
                        ))}
                    </select>
            ...
            <div className="bg-slate-100 p-1 rounded-lg flex text-sm font-medium overflow-x-auto w-full lg:w-auto">
                <button 
                    onClick={() => { setViewMode('CARDS'); initializeCardQueue(filteredWords); }}
                ...
                <button 
                    onClick={() => { setViewMode('LEARN'); resetLearnSession(filteredWords); }}
                    ...
                    <div className="bg-[#E4DBCF] rounded-2xl p-8 min-h-[500px] flex flex-col relative shadow-sm">
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center mb-8">
                            ...
                            <h3 className="text-4xl lg:text-5xl font-bold text-slate-900 mb-4 text-center">
                                {/* If Prompt is Korean */}
                                {(currentQuestionType === 'CHOICE_K_TO_N' || currentQuestionType === 'WRITING_K_TO_N') ? currentLearnItem.korean : currentLearnItem.english}
                            </h3>
                            
                            {/* Audio button if prompt is Korean */}
                            {((currentQuestionType === 'CHOICE_K_TO_N' || currentQuestionType === 'WRITING_K_TO_N') || settings.flashcard.autoTTS) && (
                                <button 
                            ...
                            {currentQuestionType.startsWith('CHOICE') ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {quizOptions.map((option, idx) => {
                                        let stateClass = "border-transparent bg-white hover:border-indigo-300 hover:bg-indigo-50";
                                        if (quizState !== 'QUESTION') {
                                            if (option.id === currentLearnItem.id) stateClass = "border-emerald-500 bg-emerald-50 text-emerald-700 font-bold ring-1 ring-emerald-500";
                                            else if (selectedOptionIdx === idx) stateClass = "border-red-500 bg-red-50 text-red-700 opacity-60";
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="text-lg font-medium text-slate-800">
                                                    {(currentQuestionType === 'CHOICE_K_TO_N') ? option.english : option.korean}
                                                </div>
                                            </button>
                                        );
                                    })}
                                ...
                                <div className="max-w-md mx-auto w-full">
                                    <form onSubmit={handleWritingSubmit} className="relative">
                                        <input
                                            ...
                                            placeholder={currentQuestionType === 'WRITING_N_TO_K' ? labels.typeAnswer : labels.typeMeaning}
                                            disabled={quizState !== 'QUESTION'}
                                            className={`w-full p-4 pr-12 text-xl font-bold text-center rounded-xl border-2 outline-none shadow-sm transition-all ${
                                                quizState === 'CORRECT' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' :
                                                quizState === 'WRONG' ? 'border-red-500 bg-red-50 text-red-700' :
                                    ...
                                    {showWritingAnswer && (
                                        <div className="mt-4 text-center animate-in slide-in-from-top-2">
                                            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{labels.correctAnswer}</div>
                                            <div className="text-xl font-bold text-indigo-600">
                                                {currentQuestionType === 'WRITING_N_TO_K' ? currentLearnItem.korean : currentLearnItem.english}
                                            </div>
                                            <button 
                        ...
                        {quizState !== 'QUESTION' && !currentQuestionType.startsWith('WRITING') && (
                            <div className={`absolute inset-x-0 bottom-0 p-4 rounded-b-2xl flex justify-center items-center gap-2 animate-in slide-in-from-bottom-2 ${quizState === 'CORRECT' ? 'bg-emerald-100/90 text-emerald-700' : 'bg-red-100/90 text-red-700'}`}>
                                {quizState === 'CORRECT' ? <CheckCircle className="w-5 h-5" /> : <XCircle className="w-5 h-5" />}
                                <span className="font-bold">{quizState === 'CORRECT' ? labels.correct : labels.incorrect}</span>
                            </div>
                        )}
                    </div>
               </div>
      ...
      {viewMode === 'LIST' && (
          <div className="max-w-4xl mx-auto space-y-4">
              {filteredWords.map((word, idx) => (
                  <div key={idx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex items-start gap-4">
                       ...
                       <div className="flex-1">
                           <div className="flex items-center gap-2 mb-1">
                               ...
                               {word.pos && <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wide ${getPosStyle(word.pos)}`}>{word.pos}</span>}
                           </div>
                       ...
                       >
                           {revealedListItems.has(word.id) ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                       </button>
                  </div>
              ))}
          </div>
